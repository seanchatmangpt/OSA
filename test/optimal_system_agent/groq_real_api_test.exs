defmodule OptimalSystemAgent.GroqRealAPITest do
  ...
  describe "Reasoning Model Token Floor — gpt-oss-20b" do
    @tag timeout: 60_000
    test "REASONING: gpt-oss-20b returns non-empty content with low max_tokens (floor enforced)" do
      # BUG: gpt-oss-20b uses ~100 internal reasoning_tokens.
      # With max_tokens: 150, reasoning consumed entire budget => EMPTY response.
      # Fix: maybe_add_max_tokens enforces floor of 500 for reasoning models.
      #
      # This test passes max_tokens: 150 to prove the floor kicks in and
      # the response is non-empty.
      messages = [
        %{
          role: "user",
          content: "What is 7 * 13? Just give the number."
        }
      ]
      ...
      # Before the fix, gpt-oss-20b returned "" with max_tokens: 150.
      assert content != nil
      assert String.trim(content) != ""
      assert String.contains?(content, "91"), "Expected answer to contain 91 (7*13), got: #{content}"
    end

    @tag timeout: 60_000
    test "REASONING: gpt-oss-20b returns non-empty content with max_tokens: 80 (floor enforced)" do
      # Even more aggressive: max_tokens: 80 should be floored to 500.
      messages = [
        %{
          role: "user",
      ...
      assert {:ok, %{content: content}} = result
      assert content != nil
      assert String.trim(content) != "", "gpt-oss-20b returned empty content with max_tokens: 80 — floor not working"
      assert String.contains?(String.downcase(content), "paris"), "Expected 'Paris' in response, got: #{content}"
    end

    @tag timeout: 60_000
    test "REASONING: gpt-oss-20b usage includes reasoning_tokens > 0" do
      # Verify the model actually uses reasoning tokens.
      messages = [
        %{
          role: "user",
          content: "If a train travels 120 km in 2 hours, what is its speed in km/h? Just the number."
        }
      ]
      ...
      assert {:ok, %{content: content} = response} = result
      assert String.trim(content) != ""
      assert String.contains?(content, "60"), "Expected 60 km/h, got: #{content}"

      # Check usage if present — reasoning_tokens should be > 0 for reasoning models
      usage = Map.get(response, :usage, %{})
      if map_size(usage) > 0 do
        # If the provider returns usage, check it's populated
        assert Map.get(usage, :output_tokens, 0) > 0 or Map.get(usage, :total_tokens, 0) > 0
      end
    end
  end
