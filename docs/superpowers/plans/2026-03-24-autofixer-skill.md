...
defmodule Superpowers.AutoFixer.FixApplierTest do
  ...
  describe "apply_fix/3" do
    test "dry_run returns changes without modifying file" do
      # Create test file with private function
      test_file = Path.join(@tmp_dir, "test.ex")
      File.write!(test_file, """
      defmodule Test do
        defp foo, do: :bar
      ...
      failure = %OutputParser{
        file: test_file,
        error_message: "** (UndefinedFunctionError) function Test.foo/0 is undefined or private"
      }
      ...
      assert {:dry_run, _changes} = result
    ...
    test "applies defp to def fix" do
      test_file = Path.join(@tmp_dir, "test.ex")
      File.write!(test_file, """
      defmodule Test do
        defp foo, do: :bar
      ...
      result = FixApplier.apply_fix(failure, pattern, dry_run: false, auto_approve: true)

      assert {:applied, _} = result
...
Expected: `** (CompileError) undefined module Superpowers.AutoFixer.FixApplier`

- [ ] **Step 3: Implement FixApplier module**
...
defmodule Superpowers.AutoFixer.FixApplier do
  ...
  def apply_fix(%OutputParser{} = failure, pattern, opts \\ []) do
    ...
    cond do
      dry_run ->
        preview_fix(failure, pattern)

      auto_approve ->
        do_apply_fix(failure, pattern)
  ...
  defp request_confirmation(failure, pattern) do
    ...
    # In automation, we default to 'n' for safety
    # Interactive mode would read from stdin
    {:skipped, changes}
  end
  ...
  # Calculate changes for defp -> def
  defp calculate_defp_to_def_changes(failure) do
    file_content = File.read!(failure.file)

    # Find all defp occurrences
    Regex.scan(~r/defp\s+([a-z_][a-z0-9_]*[!?]?)/, file_content)
    |> Enum.map(fn [full_match, func_name] ->
      %{
        file: failure.file,
  ...
  # Calculate changes for put_default helper
  defp calculate_put_default_changes(failure, pattern) do
    # This would need to parse the changeset function
  ...
  # Calculate changes for stop before start
  defp calculate_stop_before_start_changes(failure) do
    [%{
      file: failure.file,
      description: "Add GenServer.stop check before start_supervised!"
  ...
  defp apply_single_change(change) do
    content = File.read!(change.file)
    updated = String.replace(content, change.search, change.replace, global: false)
    File.write!(change.file, updated)
  end
end
```
...
```bash
git add lib/superpowers/auto_fixer/fix_applier.ex test/superpowers/auto_fixer/fix_applier_test.exs
git commit -m "feat: add fix applier for auto-fixer"
```
...
defmodule Superpowers.AutoFixer.VerificationTest do
  ...
  describe "verify_fix/2" do
    test "re-runs the test and returns result" do
      # Use a simple passing test
      failure = %OutputParser{
        test_name: "run pass",
        module: "OptimalSystemAgent.AgentTest",
        file: "test/optimal_system_agent/agent_test.exs"
      }

      result = Verification.verify_fix(failure, auto_applied: true)
...
defmodule Superpowers.AutoFixer.Verification do
  ...
  def verify_fix(%OutputParser{} = failure, opts \\ []) do
    ...
    {output, exit_code} = System.cmd("mix", ["test", test_path], cd: File.cwd!())

    pass? = exit_code == 0 and not String.contains?(output, "failure")

    if pass? do
      {:verified, :pass}
    else
      {:verification_failed, :fail, output}
    end
  end
...
```bash
git add lib/superpowers/auto_fixer/verification.ex test/superpowers/auto_fixer/verification_test.exs
git commit -m "feat: add verification engine for auto-fixer"
```
