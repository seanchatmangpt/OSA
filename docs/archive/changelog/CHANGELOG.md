...
### Added
- **Layer 12 Computer Use** (Pedro PR #2, 3,511 lines): `computer_use` tool with platform auto-detection. Linux X11 (maim + xdotool) and Linux Wayland (grim + ydotool) adapters implemented. macOS adapter scaffolded as Phase 7 stub — returns `{:error, "not yet implemented"}` until wired. Config flag `computer_use_enabled` (default: off). All non-screenshot actions classified as `:write_destructive`. Element ref targeting + coordinate fallback. Incremental tree diffs after actions.

### Changed
- **Onion audit complete** — Layers 0-9 all verified clean. No phantom modules, no dangling references, no stale supervisor children.
- **`miosa` namespace killed** — All `Miosa.*` references removed from the active codebase. `lib/miosa/shims.ex` deleted. 0 references remain. Legacy files moved to `_archived/`.

### Fixed
- Removed all `_archived/` module references from supervision trees and application startup

### Test Suite
- 1730 tests total
- 2 pre-existing failures: SQLiteStore module load (migration not run), Ollama NDJSON stream flake
- 23 tests with broken `setup_all` in MetricsTest (Telemetry config gap in `config/test.exs`)
