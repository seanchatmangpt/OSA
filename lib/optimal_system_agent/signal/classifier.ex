defmodule OptimalSystemAgent.Signal.Classifier do
  ...
  Thin wrapper around `OptimalSystemAgent.Signal.MessageClassifier` that injects the OSA
  LLM provider and event bus into the classifier's async enrichment path.

  Architecture:
  - PRIMARY: LLM classification via `OptimalSystemAgent.Providers.Registry`
  - FALLBACK: Deterministic pattern matching from `OptimalSystemAgent.Signal.MessageClassifier`
  - CACHE: ETS-backed, 10-minute TTL (managed in `OptimalSystemAgent.Signal.MessageClassifier`)
  ...
  # Re-export the struct and type from OptimalSystemAgent.Signal.MessageClassifier
  @type t :: OptimalSystemAgent.Signal.MessageClassifier.t()
  ...
  **weight** — Informational density (0.0 to 1.0):
  ...
  def classify_fast(message, channel \\ :cli) do
    OptimalSystemAgent.Signal.MessageClassifier.classify_fast(message, channel)
  end
    ...
    if llm_enabled?() do
      case call_llm(message, channel) do
        ...
        {:error, _} ->
          ...
          OptimalSystemAgent.Signal.MessageClassifier.classify_deterministic(message, channel)
      end
    else
      OptimalSystemAgent.Signal.MessageClassifier.classify_deterministic(message, channel)
    end
  ...
  """
  defdelegate calculate_weight(msg), to: OptimalSystemAgent.Signal.MessageClassifier
  ...
  defp build_signal(data, message, channel) do
    ...
    {:ok,
     %OptimalSystemAgent.Signal.MessageClassifier{
       mode: mode || :assist,
       genre: genre || :inform,
       type: type || "general",
       format: classify_format(message, channel),
       weight: weight || OptimalSystemAgent.Signal.MessageClassifier.calculate_weight(message),
       raw: message,
       channel: channel,
